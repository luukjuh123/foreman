...
async def test_get_document_wrong_owner(client: AsyncClient) -> None:
    headers_a = await _auth_headers(client, "owner@example.com")
    headers_b = await _auth_headers(client, "other@example.com")
    project_id = await _create_project(client, headers_a)
...
@pytest.mark.asyncio
async def test_update_document_metadata(client: AsyncClient) -> None:
    headers = await _auth_headers(client)
    project_id = await _create_project(client, headers)
    ...
    resp = await client.put(
        ...
        json={"name": "renamed.pdf", "category": "contract", "description": "Updated desc"},
        headers=headers,
    )
    assert resp.status_code == 200
    data = resp.json()
