...
def _make_file(content: bytes = b"fake pdf content", filename: str = "contract.pdf") -> dict:
...
async def test_list_documents_filter_by_category(client: AsyncClient) -> None:
    ...
    for name, cat in [("a.pdf", "contract"), ("b.pdf", "permit"), ("c.pdf", "contract")]:
        await client.post(
            f"/api/v1/projects/{project_id}/documents",
            files={"file": (name, io.BytesIO(b"data"), "application/pdf")},
            data={"category": cat},
...
async def test_cannot_access_other_users_document(client: AsyncClient) -> None:
    ...
    assert (await client.get(f"/api/v1/documents/{doc_id}/download", headers=h2)).status_code == 404
