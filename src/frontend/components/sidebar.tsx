      ...
      {!collapsed &&
        group.items.map(({ label, href, icon: Icon }) => {
          ...
          return (
            ...
            >
              {active && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-[3px] rounded-r-full bg-primary shadow-sm shadow-primary/40" />
              )}
              <Icon
                className={cn(
      ...
      <div className="flex items-center gap-1.5">
        {actions.map(({ label, href, icon: Icon, color }) => (
          <Link
            ...
            href={href}
            onClick={onNavigate}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-muted-foreground hover:text-foreground transition-all duration-150 group"
          >
            <div
              className={cn(
  ...
  return (
    <div className="border-t border-border/50 p-3">
      <Link
        ...
        className="flex w-full items-center gap-3 rounded-xl px-3 py-3 hover:bg-muted/50 transition-all duration-200 group"
      >
        ...
        <div className="flex flex-1 flex-col items-start min-w-0">
          ...
          <span className="text-[10px] text-primary/60 font-semibold truncate w-full text-left uppercase tracking-wide">
            Pro Plan
          </span>
        </div>
        <Settings className="h-4 w-4 text-muted-foreground/40 shrink-0 group-hover:rotate-90 transition-transform duration-300" />
      </Link>
    </div>
  );
}
...
export default function Sidebar() {
  ...
  const brand = (
    <div className="flex h-16 items-center gap-3 border-b border-border/40 px-5">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-primary/70 text-primary-foreground shadow-lg shadow-primary/30 ring-1 ring-primary/20">
