...
class StaffLoan(Base):
    ...
    deductions: Mapped[list["LoanDeduction"]] = relationship(back_populates="loan", cascade="all, delete-orphan")

    __table_args__ = (CheckConstraint("principal_cents > 0", name="ck_loan_principal_positive"),)
...
class LoanDeduction(Base):
    ...
    __table_args__ = (CheckConstraint("amount_cents > 0", name="ck_loan_deduction_amount_positive"),)
