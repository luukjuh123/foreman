...
async def _stats_for(proc: Process, db: AsyncSession) -> ProcessStats:
    """Compute stats for a single Process ORM object."""
    entry_count, total_seconds, project_count = (await db.execute(
        select(
        ...
        .where(ProjectProcess.process_id == proc.id, ProcessTimeEntry.duration_seconds.is_not(None))
    )).one()
    return ProcessStats(
        process_id=proc.id, process_slug=proc.slug, process_name=proc.name,
        entry_count=int(entry_count), project_count=int(project_count),
        total_seconds=int(total_seconds), avg_seconds=(total_seconds / entry_count) if entry_count else None,
    )


async def stats_for_process(process_id: uuid.UUID, db: AsyncSession) -> ProcessStats | None:
    """Compute stats for a single process. Returns None if the process is unknown."""
    proc = (await db.execute(select(Process).where(Process.id == process_id, Process.deleted_at.is_(None)))).scalar_one_or_none()
    return await _stats_for(proc, db) if proc else None
...
async def stats_all_processes(db: AsyncSession) -> list[ProcessStats]:
    ...
    """
    procs = (await db.execute(select(Process).where(Process.deleted_at.is_(None)).order_by(Process.slug))).scalars().all()
    return [await _stats_for(proc, db) for proc in procs]
