                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You explain construction project anomalies to a site foreman"
                            " in 1-2 sentences. Be concrete."
                        ),
                    },
...
class AnomalyDetector:
    ...
    def _over_budget(self, project: Project, ctx: ProjectContext) -> Anomaly | None:
        ...
        return self._anomaly(
            "alert.over_budget",
            "high",
            f"{project.name} is over budget",
            {"project": project.name, "budget_cents": budget, "spent_cents": ctx.spent_cents, "overrun_cents": overrun},
        )
    ...
    def _weather_risk(self, project: Project, ctx: ProjectContext) -> Anomaly | None:
        if not ctx.weather_forecast:
            return None
        start = (
            max(ctx.today, project.start_date) if project.start_date and project.start_date > ctx.today else ctx.today
        )
        ...
        return self._anomaly(
            "alert.weather_risk",
            "medium",
            f"Weather risk for {project.name}",
