...
async def generate_completion_report(
    ...
    planned_start, planned_end = base["project"]["start_date"], base["project"]["end_date"]
    timeline = {
    ...
    budget, actual = int(base["project"]["budget_cents"] or 0), int(base["totals"]["labor_cost_cents"])
    costs_vs_budget = {
        "budget_cents": budget, "actual_cost_cents": actual,
    ...
    phase_summary = [
        {
            ...
            "completed_task_count": sum(t.status == "done" for t in phase.tasks),
