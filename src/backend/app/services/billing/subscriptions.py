...
async def get_subscription(user_id: uuid.UUID, db: AsyncSession) -> Subscription | None:
    result = await db.execute(select(Subscription).where(Subscription.owner_id == user_id))
    return result.scalar_one_or_none()


async def ensure_free_subscription(user_id: uuid.UUID, db: AsyncSession) -> Subscription:
    ...
    sub = Subscription(
        ...
        project_limit=(None if trial_days > 0 else TIER_PROJECT_LIMIT[SubscriptionTier.FREE]),
        trial_ends_at=(datetime.now(UTC) + timedelta(days=trial_days) if trial_days > 0 else None),
    )
    db.add(sub)
    await db.flush()
    return sub
...
async def enforce_project_limit(user_id: uuid.UUID, db: AsyncSession) -> None:
    ...
    if current >= limit:
        raise HTTPException(
            ...
            detail=(
                f"Project limit reached for your plan ({limit}). Upgrade your subscription to create more projects."
            ),
        )
