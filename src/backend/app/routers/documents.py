...
from app.routers.deps import count_query, get_or_404, get_owned_project_or_404
...
async def upload_document(
    project_id: uuid.UUID, file: UploadFile, category: str = Form(default="other"),
    ...
    doc = Document(id=doc_id, project_id=project_id, uploaded_by=user.id, name=filename,
                   description=description, category=category, mime_type=mime_type,
                   size_bytes=size_bytes, storage_path=str(path), version=1, parent_id=None)
    db.add(doc)
    await db.commit()
...
async def list_documents(
    project_id: uuid.UUID, category: str | None = Query(default=None),
    ...
    docs = list((await db.execute(query.order_by(Document.created_at).offset(offset).limit(limit))).scalars().all())
    return DocumentListResponse(items=[DocumentResponse.model_validate(d) for d in docs], total=total)


async def _get_doc_in_project(project_id: uuid.UUID, document_id: uuid.UUID, user: User, db: AsyncSession) -> Document:
    await get_owned_project_or_404(project_id, user, db)
...
async def get_document(
    ...
    return DocumentResponse.model_validate(await _get_doc_in_project(project_id, document_id, user, db))
...
async def download_document(
    ...
    doc = await _get_doc_in_project(project_id, document_id, user, db)
    ...
    def _iter_file():
        with open(path, "rb") as f:
            while chunk := f.read(65536):
                yield chunk
    return StreamingResponse(_iter_file(), media_type=doc.mime_type,
...
@router.post("/{project_id}/documents/{document_id}/versions", response_model=DocumentResponse, status_code=status.HTTP_201_CREATED)
async def upload_new_version(
    project_id: uuid.UUID, document_id: uuid.UUID, file: UploadFile,
    description: str | None = Form(default=None),
    user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db),
) -> DocumentResponse:
    original = await _get_doc_in_project(project_id, document_id, user, db)
    versions = list((await db.execute(select(Document).where(
        (Document.id == document_id) | (Document.parent_id == document_id), Document.deleted_at.is_(None),
    ))).scalars().all())
    ...
    size_bytes = await _save_file(file, path)
    new_doc = Document(id=new_doc_id, project_id=original.project_id, uploaded_by=user.id,
                       name=filename, description=description, category=original.category,
                       mime_type=mime_type, size_bytes=size_bytes, storage_path=str(path),
                       version=next_version, parent_id=document_id)
    db.add(new_doc)
    await db.commit()
